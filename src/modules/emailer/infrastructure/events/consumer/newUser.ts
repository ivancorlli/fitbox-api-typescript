...
async function newUser() {
  const _Consumer = kafka.consumer({ groupId: 'User-Email' })
  ...
  await _Consumer.run({
    eachMessage: async ({ message }) => {
      ...
      const newUser: user = JSON.parse(msg)
      ...
      await lEmail.welcome(newUser.email)
      await lEmail.confirm(newUser.email, randomDigit, newToken)
    }
  })
}
